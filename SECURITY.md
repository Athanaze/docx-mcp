...
- **Local use (typical)**: If the MCP server runs on the **same machine** as the client (stdio or HTTP on `127.0.0.1`), **transport encryption is not this project’s job** and is usually unnecessary—the process boundary is your OS and host app. No TLS or “end-to-end encryption” layer is provided or required for that setup.
- **`DOCUMENT_ROOT`**: When set, all document paths are resolved inside this directory. Use it when the server runs in shared or multi-tenant environments so clients cannot escape the intended folder.
...
- **Secrets in env**: Do not put sensitive credentials in shell history, shared logs, or process listings.
